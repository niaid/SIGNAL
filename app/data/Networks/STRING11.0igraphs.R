...
#Working Directories
DatabaseWD <- "~/OneDrive\ -\ National\ Institutes\ of\ Health/TRIAGE/NetworkAnalysis/Update_Nov2020/"
...
#get .txt file
setwd(DatabaseWD)
String.human.full <- fread(paste0("9606.protein.links.full.v", STRING.version, ".txt"), header = T)
String.mouse.full <- fread(paste0("10090.protein.links.full.v", STRING.version, ".txt"), header = T)
...
#, vertices = c(unique(c(val[[cols[1]]], val[[cols[2]]])))
...
setwd("/Users/sakatz/TRIAGE/app/data/Networks")
