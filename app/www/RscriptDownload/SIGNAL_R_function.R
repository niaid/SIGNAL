...
# ID.column <- "EntrezID"
# criteria.column <- "normalized.signal"
# highconf.criteria <- 1
...
## [[1]] input dataframe plus 'SIGNAL.hit' column, 
## [[2]] dataframe of high confidence and medium confidence designation at each iteration, 
## [[3]] dataframe of final SIGNAL enrichments


SIGNAL.function <- function(screen.dataframe, ID.column, criteria.column, highconf.criteria, midconf.criteria, criteria.setting, enrichment.dataframe, enrichment.title, stat.test, test.cutoff, network.igraph) {
  ...
  ####---- end of while loop, Print message on completion of SIGNAL iteration
  message(paste("SIGNAL iterations complete, number of iterations: ", iteration))
  
  
  
  
  ####---- Create SIGNAL output dataframes
  input.plus.signal.df <- input.screen.dataframe
  input.plus.signal.df$SIGNAL.hit <- network.output.df$Network.hit
  ...
  signal.enrichment.df <- enrichment.step[[2]]
  
  ##-- Create List of outputs
  signal.output <- list(input.plus.signal.df, hits.by.iteration.df, signal.enrichment.df)
  
  #- Print message on list content
  message("list contents: \n [[1]] input dataframe with 'SIGNAL.hit' column, \n [[2]] dataframe of high confidence and medium confidence designation at each iteration, \n [[3]] dataframe of final SIGNAL enrichments")
