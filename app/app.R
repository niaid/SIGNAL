...
### TRIAGE veriosn
TRIAGE.version <- "Version 1.0"
TRIAGE.update.date <- "November 20, 2020"
      ...
      sidebarLayout(
        ...
        sidebarPanel(
          ...
          tags$head(
            ...
            tags$script(HTML("<script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
        ...
        mainPanel(
          tabsetPanel(id = "inTabset",
                      tabPanel(title = "Home", value = "landingPage",
                               htmlOutput("spacer1"),
                               h3("The TRIAGE platform is designed to facilitate robust hit selection from normalized high-throughput datasets.", align = "center"),
                               HTML('<center><img src="images/TRIAGE_schmematic_Home.png" width="1200"></center>'),
                               br(),
                               h4("For a detailed user guide and sample dataset click on the “User Guide” tab.", align = "center", style = "color:grey"),
                               br(),
                               br(),
                               br(),
                               HTML("<center><p>Our manuscript describing the design, development, and testing of TRIAGE can be found <a href='https://www.biorxiv.org/content/10.1101/2020.07.15.204917v2'>here</a>.</p></center>"),
                               HTML("<center><p>To cite: Katz, S., Song, J., Webb, K. P., Lounsbury, N. W., Bryant, C. E., & Fraser, I. D. C. (2020). TRIAGE: A web-based iterative analysis platform integrating pathway and network approaches optimizes hit selection from high-throughput assays. <i>bioRxiv</i>, 2020.2007.2015.204917. doi:10.1101/2020.07.15.204917</p></center>")
                               #img(src = "images/TRIAGE_schmematic_Home.png", height="75%", width="75%", align = "center")
                      ),
            ...
            tabPanel(title = "User Guide", value = "userGuide",
                     ...
                     navlistPanel(id = 'userGuideTab', widths = c(3, 6),
                                  ...
                                  tabPanel(title = "4. Reading TRIAGE results", value = "readResults",
                                  ...
                                  tabPanel(title = "Appendix C: Running TRIAGE with alternative or bespoke databases", value = "TRIAGEcode",
...
#print(data$GeneSymbol)       
        ...
        ## 'Generate_human_and_mouse_protein_codig_genes_for_TRIAGE.R' in the Rscripts folder
      ...
      observeEvent(input$goButton, {
        ...
        dir_begin <<- ifelse('SHINY_SERVER_VERSION' %in% env_names, paste0("/srv/shiny-server/data/Networks/String."), paste0("~/TRIAGE/app/data/Networks/String."))
      ...
      output$myNetworkGraph <- renderUI({
        ...
        observeEvent(input$submitButton, {
          ...
          if(length(selectedRows) == 0){
            ...
            downloadOut <- reactive({
              outputFiles = list.files(path = './')
              # reordering files so that enrichment and TRIAGE hits files come first and second  and as a seperate list on top
              ...
              nonhits.spot = grep("_nonTRIAGEhits", outputFiles)
              nonhits_outputFiles = c(outputFiles[c(nonhits.spot)])
              
              #Select files for networks
              network.spot = grep("TRIAGEnetwork", outputFiles)
              ...
              out <- c("<br><b>All files from your TRIAGE analysis for download:</b><br> <br><b><font color=blue>TRIAGE selected hits and pathway enrichments:</font></b><br>")
              ...
              if(length(nonhits_outputFiles) > 0){
                out <- paste(out, c("<br><b><font color=blue>High confidence hits that were not selected by TRIAGE:</font></b><br>"), sep = "<br><br>")
      ...
      output$sampledataset_page <- renderUI({
        tagList(HTML("<p><strong>Guide for using the sample data set for TRIAGE analysis.</strong></p>
                     <p>&nbsp;</p>
                     <p><i>Sample dataset:</i></p>"),
                downloadButton('sampleDataset', "Download a Sample TRIAGE Dataset"),
                HTML("
<p>&nbsp;</p>
...
<p>&nbsp;</p>
<ol>
<li>To run the analysis in TRIAGE, begin by accessing the TRIAGE interface in your web browser <a href='https://triage.niaid.nih.gov'>https://triage.niaid.nih.gov</a> (Chrome is the recommended browser) and downloading the dataset to your local device.</li>
...
<p>&nbsp;</p>
<ol start = '3'>
<li>Assigning a distinct value to each gene before uploading to TRIAGE and using the assigned value to separate the tiers:
<ol>
...
<p>&nbsp;</p>
<ol start='5'>
<li>Click on the &ldquo;Proteasome&rdquo; pathway box. A warning window will popup saying you are leaving an NIH website. Click OK. A new tab will open in your browser with a schematic of the proteasome. The TRIAGE identified hits from the screen are highlighted in blue (high confidence) and in red (medium confidence).</li>
<li>Return to the TRIAGE tab in your browser and click on the &ldquo;Gene Hits&rdquo; tab. The table with the &ldquo;TRIAGE Gene Hits&rdquo; lists all the hits identified by TRIAGE from the uploaded study. Additional columns show which of the other identified hits from the dataset the listed candidate has predicted interactions with and what enriched pathways the interacting genes are members of.</li>
<li>Click on the subtab labeled &ldquo;Graph: Gene Hits by Iteration&rdquo;. A table and graph show the number of high confidence and medium confidence hits selected as candidates through each iteration of TRIAGE. Iteration 0 corresponds to the settings of the upload file with 453 hits described as high confidence. After one iteration a number of the high confidence hits are dropped out and a number of the medium confidence hits are added. The cycle repeats until iteration 3 &amp; 4 that show no changes and thus the iterative analysis terminates.</li>
<li>Click on the &ldquo;High Confidence Hits Not Selected by TRIAGE&rdquo; subtab. All the hits that were designated as high confidence in the upload file but were <em><u>not </u></em>selected as hits by TRIAGE are listed in this table. This helps the user review any of the candidates that were not selected by TRIAGE that the user might want to manually add to the set of selected hits.</li>
...
<p>&nbsp;</p>")
      ...
      ####Downlaod button for sample dataset
      output$sampleDataset <- downloadHandler(
        filename = "LPS_sample_dataset.csv",
        content = function(file) {
          file.copy("www/SampleDatasets/LPS_sample_dataset.csv", file)}
      )
...
<p>&nbsp;</p>
<p>There are three requirements for a file to be successfully uploaded for analysis by TRIAGE:</p>
<p>&nbsp;</p>
<ol>
...
<p>&nbsp;</p>
<p><strong><u>File format:</u></strong> To upload your data for analysis by TRIAGE first ensure that your document is in .csv, .txt. or .xlsx format.</p>
...
<p>Both ID columns, however, do not need to be included in the upload file. If only one is included, TRIAGE will identify the missing ID type and generate a column of the other IDs based on the provided IDs. If both are provided, TRIAGE will use both columns as provided by the input file.</p>
...
<p>&nbsp;</p>
<p>Additional columns can be included in the document and they will be skipped over by the analysis but preserved in the main download file (<em><u>[name of your upload file]</u></em><em> _TRIAGEhits.csv</em>)</p>
        "
        ))
      ...
      output$runAnalysis_page <- renderUI({
        tagList(h4("Running an analysis:", align = 'center'),
          ...
          <p>&nbsp;</p>
<p>Begin by loading the TRIAGE website in your browser using the web address <a href='https://triage.niaid.nih.gov/'>https://triage.niaid.nih.gov/</a></p>
<p>&nbsp;</p>
...
<p>For the enrichment analysis portion, TRIAGE uses the pathway designations curated by the Kyoto Encyclopedia of Genes and Genomes (KEGG). (for information about the KEGG database, see <a href='https://www.kegg.jp/kegg/pathway.html'>https://www.kegg.jp/kegg/pathway.html</a><u>)</u></p>
<p>&nbsp;</p>
...
<p><strong>&nbsp;</strong></p>
<p>Type in the numeric values to be used as a cutoff for high confidence and medium confidence hits from your screen. Based on the difference between the high confidence cutoff value entered and Med-confidence cutoff value entered TRIAGE will infer whether the values should be taken as &ldquo;greater than or equal to&rdquo; or &ldquo;less than or equal to&rdquo;.</p>
<p>&nbsp;</p>
<p>(For example if 0.01 is assigned to the <em>High Confidence Cutoff Value </em>and 0.05 is assigned to the <em>Med Confidence Cutoff Value </em>field TRIAGE interprets the input as any ID with a value of 0.01 or less in the selected column should be considered high confidence, any ID with an associated value between 0.01 and 0.05 should be assigned medium confidence, and any ID with an assigned value greater than 0.05 will be assigned as a &ldquo;non-hit&rdquo;. Alternatively, if 2 is assigned to the <em>High-conf Cutoff Value </em>and 1 is assigned to the <em>Med-conf Cutoff Value </em>field TRIAGE interprets the input as any ID with a value of 2 or more in the selected column should be considered high confidence, any ID with an associated value between 1 and 2 should be assigned medium confidence, and any ID with an assigned value less than 1 will be assigned as a &ldquo;non-hit&rdquo;.</p>
<p>&nbsp;</p>
...
<p>&nbsp;</p>
<p>When selected, the added background &ldquo;genes&rdquo; will not appear as suggested hits by the TRIAGE analysis, the background genes are only used as a means to have more robust statistics on the enrichment of pathways. The added background genomes use only the known protein coding genes of the selected organisms (source: Human, HGNC. Mouse, MGI.) that are not in the upload file. TRIAGE uses the difference between the size of selected hits and the number of known protein coding genes for that organism as the number of &ldquo;non-hits&rdquo; for enrichment statistics.</p>
<p>&nbsp;</p>
...
<p>This tab allows the user to reset all the selections and restart the analysis from the start with default settings. This can be done at any point in the analysis. Clicking the reset tab will remove the uploaded file and all the analysis files generated up to that point. If trying multiple analysis or different settings it is recommended that the analysis be reset between each run by clicking the &ldquo;Reset&rdquo; icon or refreshing the browser.</p>"
      ...
      output$readResults_page <- renderUI({
        tagList(HTML(
          "<p><strong>Reading TRIAGE Results:</strong></p>
<p><strong>&nbsp;</strong></p>
...
<p><u>&nbsp;</u></p>
<p>When the analysis is complete, a list of enriched pathways appears in a table under the &ldquo;Enriched Pathways&rdquo; tab. The list includes all pathways that have a <em>p</em> value of 0.05 or less in the completed TRIAGE analysis of the uploaded data. The table also includes a list of the Gene IDs from the pathway that are also hits in the screen. The Gene IDs that were designated as high confidence in the input file are highlighted in blue, Gene IDs that were designated as medium confidence in the input file are highlighted in red.</p>
<p>&nbsp;</p>
...
<p>&nbsp;</p>
<p><strong>HitGenes: </strong>The number of hit genes as selected by the TRIAGE analysis that are in the pathway.</p>
<p>&nbsp;</p>
<p><strong>HitGeneNames: </strong>The HGNC Gene Symbols of the TRIAGE hit genes in each pathway are listed. Genes that were marked as high confidence at the start of the analysis are highlighted in blue and those that were marked as medium confidence are highlighted in red.</p>
<p>&nbsp;</p>
<p><strong><u>Gene Hits</u></strong></p>
<p><strong><u>&nbsp;</u></strong></p>
<p>The Gene Hits tab is a guide for hit selection from the uploaded data based on the analysis by TRIAGE. The Gene Hits tab is separated into five different tabs</p>
<p>&nbsp;</p>
<p>TRIAGE Gene Hits: <em>A table of all the hits selected by TRIAGE from the high and medium confidence hits in the input file.</em></p>
<p>Gene Hits by Iteration: <em>A table that lists all the hits in the input file along with what category of confidence they were assigned to at each iteration of the TRIAGE analysis.</em></p>
...
<p>High Confidence Hits Not Selected by TRIAGE: <em>A table of hits that were assigned as high confidence in the input file but were not selected as high confidence hits by TRIAGE.</em></p>
<p>Pathway Enrichments: <em>A table of pathway enrichments as assigned by TRIAGE.</em></p>
<p>&nbsp;</p>
<p><strong>TRIAGE Gene Hits: </strong>This table provides a list of hits selected by the TRIAGE analysis. The table also includes supporting information on interacting genes (based on the network criteria that were selected at the start of the analysis) and membership in enriched pathways.</p>
<p>&nbsp;</p>
<center><img src='images/TRIAGEHits_map.png' width='600'></center>
<p>&nbsp;</p>
...
<p><em>TRIAGEhit: </em>&ldquo;Yes&rdquo; indicates a hit selected by the TRIAGE analysis.</p>
...
<p><em>InteractingGenes: </em>List of other genes also selected as hits by the TRIAGE analysis that have predicted protein interactions with the assigned gene. The predicted interactions are selected based on the network criteria that was set by the user at the start of the analysis.</p>
...
<p><em>HitGenes:</em> The number of hit genes as selected by the TRIAGE analysis that are in the pathway.</p>
<p><em>HighScoreGenes:</em> The number of hit genes as selected by TRIAGE that were also categorized as high confidence based on the user provided cutoff at the start of the analysis</p>
...
<p><em>EnrichScore:</em> A calculation representing the robustness of the pathway enrichments by the number of genes represented in the TRIAGE dataset and how many of them are high scoring. The EnrichScore is calculated from &nbsp;</p>
<p>&nbsp;</p>
<p>&nbsp;</p>
<p>&nbsp;</p>
<p><strong><u>Network</u></strong></p>
<p><u>&nbsp;</u></p>
<p>TRIAGE can generate a unique interactive version of an integrated pathway and network graph. The integrated pathway and network figure provides an additional way to explore the enrichments and hits identified by TRIAGE. The user can select up to three pathways from the list of enriched pathways and TRIAGE will generate an interactive graph of all the TRIAGE hits (in the <em>Network Graph</em> tab; see below) that are part of the selected pathways as well as all the TRIAGE hits that have predicted interactions with the &lsquo;pathway&rsquo; genes. This configuration is designed to facilitate an exploration of the enrichment data that goes beyond the curated annotations of the enrichment databases. The interactive and tracking features of the figure can be used to identify possible &ldquo;missing links&rdquo; between different enrichments identified in the dataset. The analysis can also be used to identify novel pathway associations by identifying which targets from the study have predicted interactions with multiple members of an enriched pathway. The various features of this analysis can be used to further prioritize subsets of hits for validation and develop hypotheses for testing.</p>
<p>&nbsp;</p>
...
<p>&nbsp;</p>
<p>To drive further exploration of the data, the TRIAGE platform makes it possible to view which of the gene hits identified by the TRIAGE analysis that are not known members of specific gene sets (&ldquo;Novel&rdquo; genes) have predicted interactions with known members of the gene sets (&ldquo;pathways&rdquo;)</p>
<p>&nbsp;</p>
<p>&nbsp;</p>
...
<p><em>Pathway:</em> Which of all of the enriched pathways in the TRIAGE analysis the gene is a member of.</p>
<p><em>Allnet.count: </em>the number of other TRIAGE hit genes the gene has predicted interactions with (based on the user set criteria at the start)</p>
<p><em>Ntwrk.all:</em> The gene names of the genes that have predicted interactions with the individual gene.</p>
<p><em>NtwrkCount.[Name of first selected pathway]: </em>The number of genes in the first selected pathway that are hits by TRIAGE that have predicted interactions with the specific gene target.</p>
<p><em>Ntwrk.[Name of first selected pathway]: </em>The gene names of the genes from the first selected pathways that are hits by TRIAGE that have predicted interactions with the specific gene.</p>
<p><em>NtwrkCount.[Name of second selected pathway]: </em>The number of genes in the second selected pathway that are hits by TRIAGE that have predicted interactions with the specific gene target.</p>
<p><em>Ntwrk.[Name of second selected pathway]: </em>The gene names of the genes from the second selected pathways that are hits by TRIAGE that have predicted interactions with the specific gene.</p>
<p><em>NtwrkCount.[Name of third selected pathway]: </em>The number of genes in the third selected pathway that are hits by TRIAGE that have predicted interactions with the specific gene target.</p>
<p><em>Ntwrk.[Name of third selected pathway]: </em>The gene names of the genes from the third selected pathways that are hits by TRIAGE that have predicted interactions with the specific gene.</p>
<p><em>Total_Path_Hits.net.count: </em>The total number of hits by TRIAGE that are members of all of the selected pathways that have predicted interactions with the specific gene.</p>
<p>&nbsp;</p>
...
<p>&nbsp;</p>
<p><strong>[name of input file]_TRIAGEhits.csv: </strong>A table listing all the hits by TRIAGE analysis with enriched pathway and interacting network information.</p>
<p>&nbsp;</p>
<p><strong>[name of input file]_TRIAGEenrichment.csv: </strong>A table of enriched pathways identified by the analysis with their associate p-values, FDR values, Bonferroni values, and high confidence and medium confidence hit genes included.</p>
<p>&nbsp;</p>
<p><strong>[name of input file]_ nonTRIAGEhits.csv: </strong>a table listing all the genes that were assigned as high confidence hits at the start of the analysis (by the user provided criteria) but were not selected as hits by the end of the TRIAGE analysis.</p>
<p>&nbsp;</p>
<p>&nbsp;</p>
<p><strong>[name of input file]_TRIAGEnetwork_[Name(s) of selected pathways].csv:</strong> A table of all the hit genes selected by TRIAGE analysis with their predicted membership or interactions with genes in the pathways selected under the &ldquo;Network&rdquo; tab.</p>
<p>&nbsp;</p>
...
<p>&nbsp;</p>
<p><strong>Unmapped_rows: </strong>A table of all the rows with GeneSymbols that couldn&rsquo;t be mapped to EntrezIDs. (This document only shows up when the upload file only included a GeneSymbol column and EntrezID mapping was done by TRIAGE).</p>"
          )
        )
      )
...
<p><em>&nbsp;</em></p>
<p>To segment data into three tiers based on a single readout, simply assign those values to a specific column in your input file. Choose a &ldquo;stringent&rdquo; cutoff for the &ldquo;High Confidence Cutoff Value&rdquo; in TRIAGE and a more &ldquo;lenient&rdquo; cutoff for the &ldquo;Medium Confidence Cutoff Value&rdquo; in TRIAGE.</p>
<p>&nbsp;</p>
...
<p><em>&nbsp;</em></p>
<p>To segment data into three tiers based on two readouts, choose the readout that will be broken into a &ldquo;stringent&rdquo; cutoff and a &ldquo;lenient&rdquo; cutoff (Fold Change for example) assign those values to a specific column in your input file. Choose a &ldquo;stringent&rdquo; cutoff for the &ldquo;High Confidence Cutoff Value&rdquo; in TRIAGE and a more &ldquo;lenient&rdquo; cutoff for &ldquo;Medium Confidence Cutoff Value&rdquo; in TRIAGE.</p>
...
<li><strong>Assigning a distinct value to each gene before uploading to TRIAGE and using the assigned value to separate the tiers:</strong></li>
</ol>
<p><em>&nbsp;</em></p>
<p><em>This approach is most recommended when using multiple criteria or when combining the results of multiple studies. While the TRIAGE sidebar menu doesn&rsquo;t have a direct option to combine these studies the information can be combined outside of TRIAGE (such as in Excel or in R) and then run as a TRIAGE analysis.</em></p>
<p><em>&nbsp;</em></p>
...
<p>&nbsp;</p>
<p>Once the values have been added upload your file to TRIAGE. Under &ldquo;Cutoff Type&rdquo; select the name of the column in which you have added the new values. In the field &ldquo;High Confidence Cutoff Value&rdquo; enter the value 1, in the field below it (&ldquo;Medium Confidence Cutoff Value&rdquo;) enter the value 0.5. This will run the iterative analysis using the high, medium, and low confidence tiers that you&rsquo;ve assigned outside of TRIAGE.</p>"
...
<p>The TRIAGE application web interface is hosted by the <a href= 'https://www.niaid.nih.gov/about/cyber-infrastructure-computational-biology-contacts'>National Institute of Allergy and Infectious Disease (NIAID) Office of Cyber Infrastructure and Computational Biology (OCICB).</a>. The analysis of uploaded data is run behind two internet security firewalls. Access by external users to the TRIAGE interface pass through two secure firewalls. The incoming request first passes through the NIH web hosting firewall after which the analysis and TRIAGE application go through the NIAID firewall where the analysis is hosted. Migrations to external websites outside of the firewall (i.e. the KEGG interface) are accompanied with a warning message for the user.</p>
<p>TRIAGE uses a secure encrypted HTTPS connection, and all requests are handled using encrypted connections. Using these encrypted connections, only the browser from the IP address where the request originated from can access the data generated and uploaded. When a file is uploaded, a new unique directory is created where the input file and all the subsequently generated analysis files are temporarily saved, ensuring that only the user generating the connection can access the directory with their files. The directory is kept on the server only for the duration of the session (i.e. as long as the user is using the site). Once the sessions ends (i.e. close of browser window or move to a new site) the directory and all its files are removed from the TRIAGE server. This decreases the security risk for the user and ensures that the results are only stored locally after the analysis.</p>
...
<p>An additional option to increase the security of the data and generated analysis is to download TRIAGE source code and run the application locally on your own device see 'Appendix C: Running TRIAGE with alternative or bespoke databases'.</p>
<p>For any further questions about data security, please reach out to us via the &lsquo;Contact us&rsquo; tab under the &ldquo;Help&rdquo; tab.</p>"
      ...
      output$standaloneR <- renderUI(
        tagList(
          HTML("<p><strong>Appendix C: Running TRIAGE with alternative or bespoke databases</strong></p>
               <p>&nbsp;</p>"),
        HTML( "<p>To make TRIAGE an adaptable framework for iterative analysis with different datasets and databases beyond the databases and settings used on this platform, am R script version of a standalone TRIAGE function can be downloaded. The TRIAGE function relies on calling two separate analysis function, a pathway enrichment function and a network analysis function. The master TRIAGE function applies the pathway and network function iteratively, and the results are tested for when the analysis converges on a single set.</p>
          <p>&nbsp;</p>"),
          downloadButton('downloadRscript', "Download TRIAGE as a standalone R function"),
        br(),
        br(),
          HTML("
<p>The list of input variables that can be selectively assigned in the adaptable TRIAGE function in R and their required formats are:</p>
<p>&nbsp;</p>
...
<p>&nbsp;</p>
<p>The user provided variables are then used to apply the iterative function as in the previous paragraph. The adaptable version of TRIAGE broadens the scope of its application beyond the use of the specific databases and settings it was designed in.</p>
<p>&nbsp;</p>
<p>The TRIAGE function provides an output in the format of a R script list that contains three data frames:</p>
<ol>
<li>The input data frame with an appended 'TRIAGE.hit' column.</li>
<li>A data frame of high confidence and medium confidence designation at each iteration of the analysis.</li>
<li>A data frame of final TRIAGE enrichments from the provided enrichment data frame.</li>
</ol>"
        ))
      )
      
      ####Downlaod button for TRIAGE R script
      output$sampleDataset <- downloadHandler(
        filename = "TRIAGE_R_function.R",
        content = function(file) {
          file.copy("www/RscriptDownload/TRIAGE_R_function.R", file)}
      )
    ...
    output$changeLog <- renderUI({
      tagList(
        ...
        br(),
        br(),
        HTML(paste0(TRIAGE.version)),
        br(),
        HTML(paste0("Latest update: ", TRIAGE.update.date, ".")),
        br(),
        br(),
        br(),
